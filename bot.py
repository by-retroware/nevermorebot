...
async def setname(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if target_id:
            ...
            if not target_user:
                try:
                    ...
                    await add_user(chat_member.user)
                    target_user = await get_user(target_id)
                except:
                    pass
        ...
        if not target_user and nickname is None:
            ...
            target_user = await get_user(update.effective_user.id)
    
    elif update.message.reply_to_message:
        target_user = await get_user(update.message.reply_to_message.from_user.id)
        if not target_user:
            await add_user(update.message.reply_to_message.from_user)
            target_user = await get_user(update.message.reply_to_message.from_user.id)
        if context.args:
    ...
    await update_user(target_user["user_id"], "nickname", nickname)
    ...
    await add_log(update.effective_user.id, "set_name", target_user["user_id"], nickname)
...
async def giverep(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    giver = await get_user(update.effective_user.id)
    ...
    await add_log(update.effective_user.id, "giverep", uid, f"+{amount}")
...
async def warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_log(mod.id, "warn", target.id, reason)
    
    if new_warns >= 3:
        await add_mute(target.id, 1440, "Автоматический мут за 3 предупреждения", mod.id)
...
async def mute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_mute(target.id, minutes, reason, mod.id)
    ...
    await add_log(mod.id, "mute", target.id, f"{minutes}мин - {reason}")
...
async def ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_ban(target.id, 365, reason, mod.id)
    ...
    await add_log(mod.id, "ban", target.id, reason)
...
async def role(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mod_role == 0:
        ...
        await update.message.reply_text(f"✅ У *{await get_user(uid)['name']}* снята модераторская роль", parse_mode=ParseMode.MARKDOWN)
        await add_log(user_id, "role_removed", uid)
    else:
        await update_user(uid, "mod_role", mod_role)
        await update.message.reply_text(
            f"✅ *{await get_user(uid)['name']}* получил роль: {names[mod_role]} {marks[mod_role]}",
            parse_mode=ParseMode.MARKDOWN
        )
        await add_log(user_id, "role_granted", uid, f"роль {mod_role}")
...
async def giveaccess(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ *{await get_user(uid)['name']}* получил доступ: {names[level]} {marks[level]}",
        parse_mode=ParseMode.MARKDOWN
    )
    await add_log(user_id, "give_access", uid, f"уровень {level}")
...
async def nlist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in users_list[:50]:
        ...
        text += f"• {name}{username} — {u['role']} ранг{mod_role}\n"
...
async def grole(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == 0:
        ...
        await update.message.reply_text(f"✅ У *{await get_user(uid)['name']}* игровая роль сброшена до Новичка")
    else:
        await update_user(uid, "role", role)
        await update.message.reply_text(f"✅ *{await get_user(uid)['name']}* получил игровой ранг: {get_rank_name(role)}", parse_mode=ParseMode.MARKDOWN)
    
    await add_log(update.effective_user.id, "grole", uid, f"ранг {role}")
...
async def top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    users_list = sorted(await get_all_users(), key=lambda x: x["monthly_msgs"] or 0, reverse=True)[:5]
...
async def edit_nick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_log(user_id, "edit_nick", uid, f"{old_nick} -> {new_nick}")
...
async def delnick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_log(user_id, "delnick", uid, old_nick)
...
async def set_rank(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_log(user_id, "set_rank", uid, f"{old_rank}→{rank}")
...
async def reset_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_log(update.effective_user.id, "reset_user", uid, "полный сброс")
...
async def backup_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in users:
        backup_text += f"{u}\n"
    backup_text += f"\n## MUTES ({len(mutes)} записей)\n"
    for m in mutes:
        backup_text += f"{m}\n"
    backup_text += f"\n## BANS ({len(bans)} записей)\n"
    for b in bans:
        backup_text += f"{b}\n"
    backup_text += f"\n## WEDDINGS ({len(weddings)} записей)\n"
    for w in weddings:
        backup_text += f"{w}\n"
    backup_text += f"\n## LOGS ({len(logs)} записей)\n"
    for l in logs:
        backup_text += f"{l}\n"
